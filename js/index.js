	...
	*/
	...
	function check_pass() {
		...
		if (value_length >= 10 && code_length >= 3) {
			...
			$('#reg .info_pass .s4').html('¸ß').css('color', 'green');
		} else if (value_length >= 8 && code_length >= 2) {
			$('#reg .info_pass .s1').css('color', '#f60');
			$('#reg .info_pass .s2').css('color', '#f60');
			$('#reg .info_pass .s3').css('color', '#ccc');
			$('#reg .info_pass .s4').html('ÖÐ').css('color', '#f60');
		} else if (value_length >= 1) {
			$('#reg .info_pass .s1').css('color', 'maroon');
			$('#reg .info_pass .s2').css('color', '#ccc');
			$('#reg .info_pass .s3').css('color', '#ccc');
			$('#reg .info_pass .s4').html('µÍ').css('color', 'maroon');
		} else {
	...
	*/
	...
	$('form').eq(2).form('sub').click(function () {
		if (trim($('form').eq(2).form('title').value()).length <= 0 || trim($('form').eq(2).form('content').value()).length <= 0) {
			$('#blog .info').html('·¢±íÊ§°Ü£º±êÌâ»òÄÚÈÝ²»µÃÎª¿Õ£¡');
		} else {
			var _this = this;
			$('#loading').show().center(200, 40);
			$('#loading p').html('ÕýÔÚ·¢±í²©ÎÄ...');
			_this.disabled = true;
			$(_this).css('backgroundPosition', 'right');
			ajax({
				method : 'post',
